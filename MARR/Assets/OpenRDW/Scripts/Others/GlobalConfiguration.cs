...
{
    bool exec = true;
    public const float generatedPathLength =4000;//procedurally generated path length    
    public ResetTrainAgent rrc;
    public int trial = 0;
    ...
    {        
        ...
        rrc = GetComponent<ResetTrainAgent>();
    }
    ...
    public GameObject CreateNewRedirectedAvatar(int avatarId) {
        ...
        avatarId+=1;
        newAvatar.name = "Redirected Avatar" + "_" + avatarId;
        return newAvatar;
    }
    ...
    {        
        ...
        for (int id = 0; id < avatarList.Count; id++)
        {
            ...
            m_AgentGroup.RegisterAgent(rm.rrcagent);
            rm.rrcagent.set();
        }
    ...
    void EndExperiment(int endState)
    {
        int total = 0;
        for (int i = 0; i < redirectedAvatars.Count; i++) {
            var r = redirectedAvatars[i].GetComponent<RedirectionManager>();
            if(endState==0){
                // if(r.resetterChoice==RedirectionManager.ResetterChoice.RRC){
                //     Debug.Log(i+"th"+r.rrcagent.boundary_reset+"user_reset "+r.rrcagent.user_reset);
                //     total += r.rrcagent.boundary_reset;
                // }
                // else{
                    Debug.Log(i+"th"+r.boundary_reset_num+" user_reset "+r.user_reset_num);
                    total += r.boundary_reset_num;
                //}
            }
            r.boundary_reset_num=0;
            r.user_reset_num = 0;
        }
        Debug.Log("Total Reset is "+total);      
    ...
    public void GenerateTrackingSpace(int avatarNum, out List<Vector2> trackingSpacePoints, out List<List<Vector2>> obstaclePolygons, out List<InitialConfiguration> initialConfigurations)
    {
        ...
        switch (trackingSpaceChoice)
        {
            ...
            case TrackingSpaceChoice.Square://no obstacle in square
                TrackingSpaceGenerator.GenerateSquareTrackingSpace(obstacleType, out trackingSpacePoints, out obstaclePolygons, out defaultInitialConfiguration,squareWidth,squareWidth);
    ...
    public GameObject CreateAvatar(Transform trans, int avatarId){
        ...
        if( avatarRoot.GetComponent<CapsuleCollider>() == null)
            avatarRoot.AddComponent<CapsuleCollider>();
        var capsule = avatarRoot.GetComponent<CapsuleCollider>();
        capsule.height = 1.8f;
        capsule.radius = 0.5f;
        capsule.center = new Vector3(0.0f,0.9f,0.0f);
